...
16. **CEmu issue 4 DONE (2026-09-12): high-contrast screen colours.** In the user's words: *"The screen color needs to be a black background with stroke colors of white, blue, purple, and green. If more colors are necessary, cool colors get priority while warm colors are deprioritized."*
   ...
   - **Known cosmetic consequence, left as-is:** TI-OS draws its status bar as a *dark* bar with white text, so the inversion makes it the brightest large area on the screen - `#515551` before, `#737373` after (4.4:1 against black). It is about 8% of the display, and far from the 21:1 white field it replaced, but it is now the opposite of TI's intent. Darkening it would mean special-casing content, so it is flagged rather than fixed.
