...
9. **Porting both features to Windows, from source (2026-09-12). The user chose to build WabbitEmu from source rather than keep byte-patching.**
   - Everything is in `winsrc/`: `accessible-wabbitemu.patch` (+287 / -48, 17 hunks, four files), `README.md` (the build guide), and `ini/{Day,Night,Amber}/Wabbitemu.ini`.
   ...
   - **Open decision, flagged to the user:** one folder per flavour works with the patch as-is but gives each flavour its own auto-save; keeping today's single shared auto-save needs three differently-named INIs in one folder, which portable mode does not support without a further source change.
